...
def _system_prompt(card_name: str, required_fields: str) -> str:
    return (
        ...
        "score_adjustment 必须为 null，除非它引用 event_fact_sheet 中有效且未过期的 event_id，或引用 deterministic_card.factor_breakdown 中存在的 source_factors；默认幅度必须在 ±5 内，并写明 confidence。"
        "输出必须保留 deterministic_card 的结构和基础分，只允许补充 score_reasoning、action_hint、risks、evidence、confidence，并在规则允许范围内调整最终 score。"
        "risks 必须是字符串数组，不得返回对象数组。"
...
def build_long_term_prompt(fact_sheet: MarketMonitorFactSheet, deterministic_card: MarketMonitorScoreCard) -> tuple[str, str, str]:
    ...
    fields = "deterministic_score, score, zone, delta_1d, delta_5d, slope_state, recommended_exposure, factor_breakdown, score_adjustment, score_reasoning, action_hint, risks, evidence, confidence"
...
def build_short_term_prompt(fact_sheet: MarketMonitorFactSheet, deterministic_card: MarketMonitorScoreCard) -> tuple[str, str, str]:
    ...
    fields = "deterministic_score, score, zone, delta_1d, delta_5d, slope_state, factor_breakdown, score_adjustment, score_reasoning, action_hint, risks, evidence, confidence"
...
def build_system_risk_prompt(fact_sheet: MarketMonitorFactSheet, deterministic_card: MarketMonitorSystemRiskCard) -> tuple[str, str, str]:
    ...
    fields = "deterministic_score, score, zone, liquidity_stress_score, risk_appetite_score, delta_1d, delta_5d, slope_state, factor_breakdown, event_triggers, score_adjustment, score_reasoning, action_hint, risks, evidence, confidence"
...
def build_style_prompt(fact_sheet: MarketMonitorFactSheet, deterministic_card: MarketMonitorStyleEffectiveness) -> tuple[str, str, str]:
    ...
    fields = "deterministic_score, score, score_adjustment, score_reasoning, action_hint, tactic_layer, asset_layer, risks, evidence, confidence"
...
def build_panic_prompt(fact_sheet: MarketMonitorFactSheet, deterministic_card: MarketMonitorPanicCard) -> tuple[str, str, str]:
    ...
    fields = "deterministic_score, score, score_adjustment, zone, state, panic_extreme_score, selling_exhaustion_score, reversal_confirmation_score, factor_breakdown, action, system_risk_override, stop_loss, profit_rule, timeout_warning, refreshes_held, early_entry_allowed, max_position_hint, score_reasoning, action_hint, risks, evidence, confidence"
