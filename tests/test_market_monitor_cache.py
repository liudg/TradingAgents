import unittest
from datetime import date
from pathlib import Path
from tempfile import TemporaryDirectory
from unittest.mock import patch

import pandas as pd

from tradingagents.web.market_monitor import cache, io_utils


class MarketMonitorCacheTests(unittest.TestCase):
    def test_load_symbol_daily_cache_ignores_unrelated_files(self) -> None:
        with TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            market_monitor_dir = temp_path / "market_monitor"
            data_dir = market_monitor_dir / "data"
            data_dir.mkdir(parents=True)

            legacy_file = temp_path / "SPY-YFin-data-2026-04-10.csv"
            pd.DataFrame(
                {
                    "Date": ["2026-04-09", "2026-04-10"],
                    "Close": [100.0, 101.0],
                }
            ).to_csv(legacy_file, index=False)

            with patch.object(cache, "MARKET_MONITOR_DATA_DIR", data_dir):
                frame = cache.load_symbol_daily_cache("SPY", date(2026, 4, 10))

            self.assertTrue(frame.empty)

    def test_save_symbol_daily_cache_writes_into_data_subdirectory(self) -> None:
        with TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            data_dir = temp_path / "market_monitor" / "data"
            data_dir.mkdir(parents=True)
            frame = pd.DataFrame(
                {
                    "Open": [1.0],
                    "High": [2.0],
                    "Low": [0.5],
                    "Close": [1.5],
                    "Volume": [100],
                },
                index=pd.to_datetime(["2026-04-10"]),
            )

            with patch.object(cache, "MARKET_MONITOR_DATA_DIR", data_dir):
                cache.save_symbol_daily_cache("XLE", frame)

            saved = pd.read_csv(data_dir / "XLE.csv")
            self.assertEqual(saved.loc[0, "Close"], 1.5)

    def test_json_atomic_write_retries_replace_after_permission_error(self) -> None:
        with TemporaryDirectory() as temp_dir:
            target = Path(temp_dir) / "payload.json"
            call_count = {"replace": 0}
            original_replace = Path.replace

            def flaky_replace(src: Path, dest: Path) -> Path:
                if src.suffix == ".tmp" and dest.name == "payload.json":
                    call_count["replace"] += 1
                    if call_count["replace"] == 1:
                        raise PermissionError(5, "拒绝访问。")
                return original_replace(src, dest)

            with patch.object(io_utils.Path, "replace", new=flaky_replace):
                io_utils.write_json_atomic(target, {"status": "ok"})

            self.assertEqual(call_count["replace"], 2)
            self.assertIn('"status": "ok"', target.read_text(encoding="utf-8"))

    def test_csv_atomic_write_retries_replace_after_permission_error(self) -> None:
        with TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            data_dir = temp_path / "market_monitor" / "data"
            data_dir.mkdir(parents=True)
            frame = pd.DataFrame(
                {
                    "Open": [1.0],
                    "High": [2.0],
                    "Low": [0.5],
                    "Close": [1.5],
                    "Volume": [100],
                },
                index=pd.to_datetime(["2026-04-10"]),
            )
            call_count = {"replace": 0}
            original_replace = Path.replace

            def flaky_replace(src: Path, dest: Path) -> Path:
                if src.suffix == ".tmp" and dest.name == "XLE.csv":
                    call_count["replace"] += 1
                    if call_count["replace"] == 1:
                        raise PermissionError(5, "拒绝访问。")
                return original_replace(src, dest)

            with patch.object(cache, "MARKET_MONITOR_DATA_DIR", data_dir), patch.object(
                io_utils.Path,
                "replace",
                new=flaky_replace,
            ):
                cache.save_symbol_daily_cache("XLE", frame)

            self.assertEqual(call_count["replace"], 2)
            saved = pd.read_csv(data_dir / "XLE.csv")
            self.assertEqual(saved.loc[0, "Close"], 1.5)


if __name__ == "__main__":
    unittest.main()
